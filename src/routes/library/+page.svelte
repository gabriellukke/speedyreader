		...
		{:else}
			<div class="grid gap-4 md:gap-6">
				{#each items as item (item.id)}
					<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 md:p-6 hover:shadow-xl transition-shadow">
						<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
							<div class="flex-1 min-w-0">
								<h3 class="text-lg md:text-xl font-semibold text-gray-900 dark:text-white mb-2 truncate">
									{item.title}
								</h3>
								<div class="flex flex-wrap gap-3 md:gap-4 text-xs md:text-sm text-gray-600 dark:text-gray-400">
